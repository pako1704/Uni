...
public class BankBackup {
  ...
  public BankBackup(int bankCode) {
  ...
  public boolean removeAccount(int accountNumber) {
    ...
    // If number of accounts is too low, shrink array while moving accounts to
    // the left
    if (accounts.length != 8 && currentIndex < accounts.length / 8) {
      /*
       * Offset used to determine if the loop is currently in front of the hole
       * or after it. If it is in front of the hole, just copy the current
       * account to the new array. Else, copy the next account to move all
  ...
  // Returns the account with the given number, if it exists, else it returns
  // null;
